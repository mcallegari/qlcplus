...
function(i, rx, ry, r, g, b) {
  ...
  var factor = Math.sqrt(percentX * percentX / saturation) + Math.sqrt(percentY * percentY / saturation);
...
function(i, rx, ry, r, g, b) {
  ...
  var factor1 =
  Math.max(0, Math.min(1, 1 - (Math.sqrt((offx * offx) + (1.5 * offy * offy)) / (snowmanAlgo.cache.size1 + 1))));

  // Ball 2
  offy = ry - algo.obj[i].y + snowmanAlgo.cache.yOffset2;
  var factor2 =
  Math.max(0, Math.min(1, 1 - (Math.sqrt((offx * offx) + (offy * offy)) / (snowmanAlgo.cache.size2 + 1))));

  // Ball 3
  offy = ry - algo.obj[i].y + snowmanAlgo.cache.yOffset3;
  var factor3 =
  Math.max(0, Math.min(1, 1 - (Math.sqrt((offx * offx) + (1.5 * offy * offy)) / (snowmanAlgo.cache.size3 + 1))));
...
function(i, rx, ry, r, g, b) {
  ...
  if (algo.presetSize < 10) {
    ...
    var distPercent = distance / algo.presetRadius;
    factor = baseIntensity + (1 - baseIntensity) * (Math.abs(colorAngle - Math.PI) / algo.twoPi + (1 - distPercent));
...
ventilatorAlgo.getMapPixelColor =
function(i, rx, ry, r, g, b) {
  var factor = 1.0;
  // calculate the offset difference of algo.map location to the float
  // location of the object
...
[ "Candle", candleAlgo ],               // 2
[ "Circle", circleAlgo ],               // 3
[ "Diamond", diamondAlgo ],             // 4
[ "Disk", diskAlgo ],                   // 5
[ "Eye", eyeAlgo ],                     // 6
[ "Flower", flowerAlgo ],               // 7
[ "Heart", heartAlgo ],                 // 8
[ "Hexagon", hexagonAlgo ],             // 9
[ "Mask", maskAlgo ],                   // 10
[ "Pentagon", pentagonAlgo ],           // 11
[ "Ring", ringAlgo ],                   // 12
[ "Snowflake", snowflakeAlgo ],         // 13
[ "Snowman", snowmanAlgo ],             // 14
[ "Square", squareAlgo ],               // 15
[ "Star", starAlgo ],                   // 16
[ "Steeringwheel", steeringwheelAlgo ], // 17
[ "Tornado", tornadoAlgo ],             // 18
[ "Tree", treeAlgo ],                   // 19
[ "Triangle", triangleAlgo ],           // 20
[ "Ufo", ufoAlgo ],                     // 21
[ "Ventilator", ventilatorAlgo ]        // 22
);
...
algo.properties.push("name:presetCollision|type:list|display:Self Collision|" +
...
function(tips) {
  if (tips === 6) {
    ...
    geometryCalc.cache.innerRadius = 0.688 * algo.presetRadius / 0.851;
  } else if (tips === 4) {
    geometryCalc.cache.innerRadius = algo.presetRadius * 0.707;
  } else { // tips === 3
    geometryCalc.cache.innerRadius = algo.halfRadius;
  }
...
function(angle, tips) {
  ...
  var anglePart = (angle + geometryCalc.cache.r) % (algo.twoPi / tips) - geometryCalc.cache.r;
  var targetDistance = geometryCalc.cache.innerRadius / Math.cos(anglePart);

  return targetDistance;
}

// calculate the angle from 0 to 2 pi starting north and counting clockwise
geometryCalc.getAngle =
function(offx, offy) {
  var angle = 0;
  // catch offx == 0
  if (offx == 0) {
...
util.blindoutPercent =
function(percent, sharpness) {
  ...
  // asec consumes values > 1. asec(x) = acos(1/x)
  var factor = Math.min(1, Math.acos(1 / (Math.sqrt(sharpness * percent * percent) + 1)) * algo.halfPi);
  return factor;
}
