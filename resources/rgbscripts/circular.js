...
"name:circularMode|type:list|display:Mode|values:Radar,Spiral Right,Spiral Left,S-Curve Right,S-Curve Left,Rings Spreading,Rings Rotating|write:setMode|read:getMode");
algo.fillMatrix = 0;
...
algo.centerRadius = 0;
algo.properties.push(
"name:centerRadius|type:range|display:Center Rotation|values:-10,10|write:setCenterRotation|read:getCenterRotation");
algo.fadeMode = 0;
algo.properties.push(
"name:fadeMode|type:list|display:Radar Fade Mode|values:Don't Fade,Fade Left,Fade Right|write:setFade|read:getFade");
...
algo.setMode = function(_mode) {
  if (_mode === "Spiral Right") {
    algo.circularMode = 1;
  } else if (_mode === "Spiral Left") {
    algo.circularMode = 2;
  } else if (_mode === "S-Curve Right") {
    algo.circularMode = 3;
  } else if (_mode === "S-Curve Left") {
    algo.circularMode = 4;
  } else if (_mode === "Rings Spreading") {
    algo.circularMode = 5;
  } else if (_mode === "Rings Rotating") {
    algo.circularMode = 6;
  } else {
    algo.circularMode = 0;
  }
...
function(percent, sharpness) {
  ...
  var factor = Math.min(1, Math.acos(1 / (Math.sqrt(sharpness * percent * percent) + 1)) * util.halfPi);
  return factor;
}

util.step =
function(width, height, rgb, step) {
  // clear algo.map data
  for (var y = 0; y < height; y++) {
    for (var x = 0; x < width; x++) {
      util.map[y][x] = 0;
...
util.getMapPixelColor =
function(ry, rx, r, g, b) {
  var factor = 1.0;
