...
"name:StarsAmount|type:range|display:Number of Stars (10-255)|values:10,255|write:setAmount|read:getAmount");
algo.presetStars = 50; // 50 stars on screen at one time (default)
algo.properties.push(
"name:MultiColor|type:list|display:MultiColored Stars?|values:No,Yes|write:setMulti|read:getMulti");
algo.multiColor = 0; // Multicolor stars off defaultly (1 = stars will be randomly colored)
algo.properties.push(
"name:InvertBrightness|type:list|display:Invert Brightness|values:Dim->Bright,Bright->Dim|write:setInvert|read:getInvert");
...
algo.rgbMap = function(width, height, rgb, step) {
  ...
  for (var i = 0; i < algo.presetStars - 1; i++) {
    ...
    if (height >= width) {
      stars[i].z -= height / (height / 4);
    } else {
      stars[i].z -= width / (width / 4);
    }
    ...
    if (px >= 0 && px < width && py >= 0 && py < height) {
      ...
      // console.log("i: " + i + " k: " + k + " px: " + px + " width: " + width + " py: " + py + " height: " +
      // height);
