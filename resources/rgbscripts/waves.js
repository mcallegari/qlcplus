...
"name:direction|type:list|display:Direction|values:Right,Left,In,Out|write:setDirection|read:getDirection");
algo.orientation = 0;
algo.properties.push(
...
algo.getDirection = function() {
  if (algo.direction === 0) {
    return "Right";
  } else if (algo.direction === 1) {
    return "Left";
  } else if (algo.direction === 2) {
    return "In";
  } else if (algo.direction === 3) {
    return "Out";
  }
};
    ...
    for (var x = 0; x < width; x++) {
      ...
      case 3: // Out
        if (stepPos <= center) {
          ...
          fill = (((span - 1 - stepPos) <= step) && ((span - 1 - stepPos) > (step - tailSteps)));
        }
        break;
      }
