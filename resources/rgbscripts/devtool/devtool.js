...
function(property) {
  ...
  if (typeProperty === "list") {
    ...
    input.name = name;
    input.id = name;
    input.setAttribute("onChange", "devtool.writeFunction('" + writeFunction + "', '" + name
                                   + "', this.value); devtool.setStep(0); devtool.writeCurrentStep()");
...
function() {
  devtool.currentStep =
  parseInt(document.getElementById("currentStep").value); // currentStep may have been changed manually
  ...
  for (var y = 0; y < devtool.gridheight; y++) {
    ...
    for (var x = 0; x < devtool.gridwidth; x++) {
      ...
      cell.title =
      "(" + x + ", " + y + "): " + rgbStr + " – "
      + cell.style.backgroundColor; // rgbStr will be #rrggbb whereas the cell style will be rgb(255, 255, 255)
    }
  }
}
...
// and there are scripts changing this number on any parameter
// change
devtool.stepCount =
function() {
...
devtool.handleLoadError =
function() {
  return;
}

devtool.onFilenameUpdated =
function(filename) {
  if (filename === "") {
    return;
  }
...
devtool.writeFunction =
function(functionName, propertyName, value) {
  window.testAlgo[functionName](value);
  localStorage.setItem(propertyName, value);
  devtool.updateStepCount();
}

devtool.onAlternateChanged =
function() {
