...
"name:linesVariability|type:range|display:Size Variablity|values:0,100|write:setVariability|read:getVariability");
algo.linesType = 0;
algo.properties.push(
"name:linesType|type:list|display:Type|values:Horizontal,Vertical,Plus,X,Star,Left,Right,Up,Down,UpRight,UpLeft,DownRight,DownLeft|write:setType|read:getType");
algo.linesBias = 0;
algo.properties.push(
"name:linesBias|type:list|display:Starting Bias|values:Random,Top,Bottom,Left,Right,TopLeft,TopRight,BottomLeft,BottomRight|write:setBias|read:getBias");
algo.linesSlide = 0;
algo.properties.push(
"name:linesSlide|type:list|display:Slide|values:None,Up,Down,Left,Right|write:setSlide|read:getSlide");
...
algo.fadeMode = 0;
algo.properties.push(
"name:fadeMode|type:list|display:Fade Mode|values:Don't Fade,Fade In,Fade Out|write:setFade|read:getFade");
...
algo.setType = function(_type) {
  if (_type === "Vertical") {
    ...
    algo.linesType = 8;
  } else if (_type === "UpRight") {
    algo.linesType = 9;
  } else if (_type === "UpLeft") {
    algo.linesType = 10;
  } else if (_type === "DownRight") {
    algo.linesType = 11;
  } else if (_type === "DownLeft") {
    algo.linesType = 12;
  } else {
    algo.linesType = 0;
  }
...
algo.getType = function() {
  if (algo.linesType === 1) {
    return "Vertical";
  } else if (algo.linesType === 2) {
    return "Plus";
  } else if (algo.linesType === 3) {
    return "X";
  } else if (algo.linesType === 4) {
    return "Star";
  } else if (algo.linesType === 5) {
    return "Left";
  } else if (algo.linesType === 6) {
    return "Right";
  } else if (algo.linesType === 7) {
    return "Up";
  } else if (algo.linesType === 8) {
    return "Down";
  } else if (algo.linesType === 9) {
    return "UpRight";
  } else if (algo.linesType === 10) {
    return "UpLeft";
  } else if (algo.linesType === 11) {
    return "DownRight";
  } else if (algo.linesType === 12) {
    return "DownLeft";
  } else {
    return "Horizontal";
  }
...
algo.getSlide = function() {
  if (algo.linesSlide === 1) {
    return "Up";
  } else if (algo.linesSlide === 2) {
    return "Down";
  } else if (algo.linesSlide === 3) {
    return "Left";
  } else if (algo.linesSlide === 4) {
    return "Right";
  } else {
    return "None";
  }
...
algo.setBias = function(_bias) {
  if (_bias === "Top") {
    algo.linesBias = 1;
  } else if (_bias === "Bottom") {
    algo.linesBias = 2;
  } else if (_bias === "Left") {
    algo.linesBias = 3;
  } else if (_bias === "Right") {
    algo.linesBias = 4;
  } else if (_bias === "TopLeft") {
    algo.linesBias = 5;
  } else if (_bias === "TopRight") {
    algo.linesBias = 6;
  } else if (_bias === "BottomLeft") {
    algo.linesBias = 7;
  } else if (_bias === "BottomRight") {
    algo.linesBias = 8;
  } else {
    algo.linesBias = 0;
  }
...
algo.getBias = function() {
  if (algo.linesBias === 1) {
    return "Top";
  } else if (algo.linesBias === 2) {
    return "Bottom";
  } else if (algo.linesBias === 3) {
    return "Left";
  } else if (algo.linesBias === 4) {
    return "Right";
  } else if (algo.linesBias === 5) {
    return "TopLeft";
  } else if (algo.linesBias === 6) {
    return "TopRight";
  } else if (algo.linesBias === 7) {
    return "BottomLeft";
  } else if (algo.linesBias === 8) {
    return "BottomRight";
  } else {
    return "Random";
  }
...
util.getNextStep = function(width, height, rgb) {
  ...
  for (var i = 0; i < algo.linesAmount; i++) {
    ...
    if (algo.linesSlide > 0) {
      ...
      if (algo.linesRollover == 1) {
        if (lines[i].xCenter == 0) {
          lines[i].xCenter = width;
        }
        if (lines[i].yCenter == 0) {
          lines[i].yCenter = height;
        }
        if (lines[i].xCenter >= width) {
          lines[i].xCenter = 0;
        }
        if (lines[i].yCenter >= height) {
          lines[i].yCenter = 0;
        }
      }
    }
    if (lines[i].step >= util.linesMaxSize
        || Math.floor(Math.random() * 100 + (lines[i].step * 2)) < algo.linesVariability) {
      lines[i].xCenter = -1;
      lines[i].yCenter = -1;
      lines[i].step = 0;
    }
