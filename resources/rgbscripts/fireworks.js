...
"name:particleCount|type:range|display:Particles|values:5,30|write:setParticleCount|read:getParticleCount");
algo.particleSteps = 15;
algo.properties.push(
"name:particleSteps|type:range|display:Size|values:5,30|write:setParticleSteps|read:getParticleSteps");
...
algo.rgbMap = function(width, height, rgb, progstep) {
  if (algo.initialized === false || util.map.length != height || util.map[0].length != width) {
  ...
  for (var i = 0; i < algo.rocketsCount; i++) {
    ...
    if (!algo.rockets[i].initialized || algo.rockets[i].particle.length < algo.particleCount) {
      util.initializeRocket(i, width, height, rgb);
    }

    // Trigger the rocket
    if (!algo.rockets[i].triggered && Math.floor(algo.rockets[i].y) <= Math.floor(algo.rockets[i].triggerPoint)) {
    ...
    if (!algo.rockets[i].triggered) {
      // Draw the rocket
      util.drawObject(algo.rockets[i].x, algo.rockets[i].y, width, height, algo.rockets[i].r, algo.rockets[i].g,
                      algo.rockets[i].b);
    } else {
      ...
      for (var j = 0; j < algo.particleCount; j++) {
        ...
        algo.rockets[i].particle[j].x += algo.rockets[i].particle[j].xDirection;
        algo.rockets[i].particle[j].y -= algo.rockets[i].particle[j].yDirection - util.gravity;
        // draw the particles
        util.drawObject(algo.rockets[i].particle[j].x, algo.rockets[i].particle[j].y, width, height, algo.rockets[i].r,
                        algo.rockets[i].g, algo.rockets[i].b);
      }
    }
    ...
    algo.rockets[i].xDirection = algo.rockets[i].xDirection * (1 + algo.rockets[i].yDirection / 30);
  }
  return util.map;
};
...
function(i, w, h, rgb) {
  ...
  algo.rockets[i].xDirection = (algo.rockets[i].x - (w / 2)) / w * util.getNewNumberRange(-1, 1);
