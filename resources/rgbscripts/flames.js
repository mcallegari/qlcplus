...
;(function () {
  ...
  algo.properties.push(
    'name:fireAmount|type:range|display:Amount|values:1,200|write:setAmount|read:getAmount'
  )
  ...
  algo.fireDepth = 40
  algo.properties.push(
    'name:fireDepth|type:range|display:Depth|values:1,100|write:setDepth|read:getDepth'
  )
    ...
    for (y = height; y > -1; y--) {
      if (y === height) {
        ...
        for (i = 0; i < count; i++) {
          ...
          fire[y][x] =
            (fire[y][x] + Math.floor(0x80 + Math.random() * 0x5f)) % 0xff
        }
      } else {
        for (x = 0; x < width; x++) {
          //console.log('pos:'+y+','+x)
          ...
          fire[y][x] =
            Math.floor(
              // add the cell values:
              (fire[(y + 1) % (height + 1)][(x - 1) % width] * 0.9 + // below, left
                fire[(y + 1) % (height + 1)][x] + // below
                fire[(y + 1) % (height + 1)][(x + 1) % width] * 0.9 + // below, right
                fire[(y + 2) % (height + 1)][(x - 1) % width] * 0.8 + // two rows below, left
                fire[(y + 2) % (height + 1)][x] * 0.8 + // two rows below
                fire[(y + 2) % (height + 1)][(x + 1) % width] * 0.8 + // two rows below, right
                fire[(y + 2) % (height + 1)][(x + 2) % width] * 0.7 + // two rows below, two to the left
                fire[(y + 2) % (height + 1)][(x - 2) % width] * 0.7) / // two rows below, two to the right
                8 +
                algo.fireDepth / 10
            ) || fire[y][x] // division to lower the value as the fire goes up
