...
DMXUSBOpenRx::DMXUSBOpenRx(DMXInterface *_interface,
                                   quint32 inputLine, QObject* parent)
    : QThread(parent)
    , DMXUSBWidget(_interface, 0, DEFAULT_OPEN_DMX_FREQUENCY)
    , m_running(false)
...
void DMXUSBOpenRx::run()
{
    ...
    {
        payload = _interface()->read(RX_BUFFER_SIZE);
        ...
        else
        {
            ...
            {
                qDebug() << _interface()->serial() << "Bogus frame" << current_payload.length() << "bytes instead of" << last_payload.length();
                current_payload.clear();
                erroneous_frames += 1;
                continue;
            }
            ...
            if (missed_frames > RECEIVE_START_THRESHOLD) // only to emit the debug message once, not at each frame
                qDebug() << _interface()->serial() << "Receiving";
        ...
        if (missed_frames == RECEIVE_START_THRESHOLD)
        {
            ...
            qDebug() << _interface()->serial() << "Idling";
        } 
    ...
    qDebug() << _interface()->serial() << "Requested to stop";
    _interface()->setLowLatency(false);
    m_reader_state = Calibrating;
}
