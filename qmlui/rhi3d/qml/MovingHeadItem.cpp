    ...
    : MeshItem(parent)
{
    m_path = QDir::cleanPath(QLCFile::systemDirectory(MESHESDIR).path()
                             ...
                             + QDir::separator() + "moving_head.glb");
}
...
void MovingHeadItem::setPath(const QString &path)
{
    ...
    if (path.isEmpty())
    {
        qWarning() << "MovingHeadItem: empty mesh path";
    }
    else if (!QFileInfo::exists(localPathForFileCheck(path)))
    {
        qWarning() << "MovingHeadItem: mesh path does not exist:" << path;
    }
...
void MovingHeadItem::setBeamMode(bool enabled)
{
...
void MovingHeadItem::setBeamRadius(float radius)
{
...
Light MovingHeadItem::toLight() const
{
    ...
    if (m_beamMode)
    {
        // BeamShape is evaluated cylindrically in the lighting shader, but
        // a few auxiliary paths still rely on spot-cone parameters.
        // Use an almost-flat frustum so any residual cone apex offset is
        // effectively zero-height at the emitter.
        const float halfAngle = qDegreesToRadians(85.0f);
        light.outerCone = halfAngle;
        light.innerCone = qDegreesToRadians(80.0f);
    }
    else
    {
