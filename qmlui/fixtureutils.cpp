...
QColor FixtureUtils::headColor(Fixture *fixture, int headIndex)
{
    ...
    if (rgbCh.size() == 3)
    {
        finalColor.setRgb(fixture->channelValueAt(rgbCh.at(0)), fixture->channelValueAt(rgbCh.at(1)),
    ...
    if (cmyCh.size() == 3)
    {
        finalColor.setCmyk(fixture->channelValueAt(cmyCh.at(0)), fixture->channelValueAt(cmyCh.at(1)),
...
QColor FixtureUtils::applyColorFilter(QColor source, QColor filter)
{
    ...
    return QColor(source.redF() * filter.redF() * 255.0, source.greenF() * filter.greenF() * 255.0,
