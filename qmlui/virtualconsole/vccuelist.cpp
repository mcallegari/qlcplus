...
void VCCueList::adjustIntensity(qreal val)
{
    ...
    if (ch != nullptr)
    {
        ...
        {
            ch->adjustStepIntensity(qreal(m_sideFaderLevel) / 100,
                                    m_primaryTop ? m_playbackIndex : m_nextStepIndex);
            ch->adjustStepIntensity(qreal(100 - m_sideFaderLevel) / 100,
                                    m_primaryTop ? m_nextStepIndex : m_playbackIndex);
        }
    }
...
void VCCueList::setSideFaderLevel(int level)
{
    ...
    else
    {
        ...
        if (!(ch == nullptr || ch->stopped()))
        {
            ch->adjustStepIntensity(qreal(level) / 100.0,
                                    m_primaryTop ? m_playbackIndex : m_nextStepIndex,
                                    Chaser::FadeControlMode(getFadeMode()));
            ch->adjustStepIntensity(qreal(100 - level) / 100.0,
                                    m_primaryTop ? m_nextStepIndex : m_playbackIndex,
                                    Chaser::FadeControlMode(getFadeMode()));
            stopStepIfNeeded(ch);
        }
...
{
    if (sideFaderMode() == FaderMode::None || sideFaderMode() == FaderMode::Steps)
        return 1.0;
...
void VCCueList::setChaserID(quint32 fid)
{
    ...
    if (current != nullptr)
    {
        ...
        disconnect(current, SIGNAL(currentStepChanged(int)), this, SLOT(slotCurrentStepChanged(int)));
        connect(current, SIGNAL(stepChanged(int)), this, SLOT(slotStepChanged(int)));
    ...
    if (function != nullptr)
    {
        ...
        connect(function, SIGNAL(currentStepChanged(int)), this, SLOT(slotCurrentStepChanged(int)));
        connect(function, SIGNAL(stepChanged(int)), this, SLOT(slotStepChanged(int)));
    ...
    if (step.m_function != NULL)
    {
        ...
        if (step.m_duration == Function::infiniteSpeed())
        {
            if (status == ProgressFadeIn && step.m_fadeIn != Function::defaultSpeed())
            {
                ...
                m_item->setProperty("progressText", QString("-%1").arg(Function::speedToString(
                                                        step.m_fadeIn - step.m_elapsed)));
            }
            else
            {
        ...
        else
        {
            ...
            m_item->setProperty("progressValue", progress);
            m_item->setProperty("progressText", QString("-%1").arg(Function::speedToString(
                                                    step.m_duration - step.m_elapsed)));
        }
    }
    else
    {
