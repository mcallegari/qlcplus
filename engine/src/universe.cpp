    ...
    , m_passthroughValues()
{
    ...
    connect(m_grandMaster, SIGNAL(valueChanged(uchar)), this, SLOT(slotGMValueChanged()));
}
...
void Universe::setPassthrough(bool enable)
{
    ...
    if (enable && m_passthroughValues.isNull())
    {
        ...
        // When enabling passthrough, make sure the array is allocated BEFORE m_passthrough is set
        // to true. That way we only have to check for m_passthrough, and do not need to check
...
QSharedPointer<GenericFader> Universe::requestFader(Universe::FaderPriority priority)
{
    ...
    qDebug() << "Generic fader with priority" << fader->priority() << "registered at pos"
             << insertPos << ", count" << m_faders.count();
...
void Universe::requestFaderPriority(QSharedPointer<GenericFader> fader, Universe::FaderPriority priority)
{
    ...
    if (newPos != pos)
    {
        ...
        qDebug() << "Generic fader moved from" << pos << "to" << m_faders.indexOf(fader)
                 << ". Count:" << m_faders.count();
    }
}
...
void Universe::reset(int address, int range)
{
    ...
    memcpy(m_postGMValues->data() + address, m_modifiedZeroValues->data() + address,
           range * sizeof(*m_postGMValues->data()));
...
void Universe::applyPassthroughValues(int address, int range)
{
    ...
    {
        if (static_cast<uchar>(m_postGMValues->at(i))
            < static_cast<uchar>(m_passthroughValues->at(i))) // HTP merge
        {
...
bool Universe::setInputPatch(QLCIOPlugin *plugin, quint32 input, QLCInputProfile *profile)
{
    qDebug() << "[Universe] setInputPatch - ID:" << m_id
             << ", plugin:" << ((plugin == NULL) ? "None" : plugin->name()) << ", input:" << input
...
void Universe::setChannelCapability(ushort channel, QLCChannel::Group group, ChannelType forcedType)
{
    ...
    // qDebug() << Q_FUNC_INFO << "Channel:" << channel << "mask:" <<
    // QString::number(m_channelsMask->at(channel), 16);
...
bool Universe::write(int channel, uchar value, bool forceLTP)
{
    ...
    if (forceLTP == false && (m_channelsMask->at(channel) & HTP)
        && value < (uchar)m_preGMValues->at(channel))
    {
...
bool Universe::writeBlended(int channel, uchar value, Universe::BlendMode blend)
{
    ...
    switch (blend)
    {
        ...
        case AdditiveBlend:
        {
            ...
            // qDebug() << "Universe write additive channel" << channel << ", value:" << currVal <<
            // "+" << value;
...
bool Universe::saveXML(QXmlStreamWriter *doc) const
{
    ...
    if (inputPatch() != NULL)
    {
        savePatchXML(doc, KXMLQLCUniverseInputPatch, inputPatch()->pluginName(),
                     inputPatch()->inputName(), inputPatch()->input(), inputPatch()->profileName(),
                     inputPatch()->getPluginParameters());
    }
    foreach (OutputPatch *op, m_outputPatchList)
    {
    ...
    if (feedbackPatch() != NULL)
    {
        savePatchXML(doc, KXMLQLCUniverseFeedbackPatch, feedbackPatch()->pluginName(),
                     feedbackPatch()->outputName(), feedbackPatch()->output(), "",
                     feedbackPatch()->getPluginParameters());
    }
...
void Universe::savePatchXML(QXmlStreamWriter *doc, const QString &tag, const QString &pluginName,
...
bool Universe::savePluginParametersXML(QXmlStreamWriter *doc, QMap<QString, QVariant> parameters) const
{
    Q_ASSERT(doc != NULL);
