...
Show::Show(Doc *doc)
    : Function(doc, Function::ShowType)
...
{
    while (m_tracks.contains(m_latestTrackId) == true || m_latestTrackId == Track::invalidId())
    {
        m_latestTrackId++;
    }
...
int Show::adjustAttribute(qreal fraction, int attributeId)
{
    ...
    if (m_runner != NULL)
    {
        ...
        if (trkList.isEmpty() == false && attrIndex >= 0 && attrIndex < trkList.count())
        {
