...
void EFXFixture::durationChanged()
{
    ...
    m_elapsed = SCALE(float(m_currentAngle),
            ...
            float(0), float(m_parent->innerSpeeds().duration()));
    ...
    if (timeOffset())
    {
        if (m_elapsed < timeOffset())
            m_elapsed += m_parent->innerSpeeds().duration();
...
uint EFXFixture::timeOffset() const
{
    ...
    {
        return m_parent->innerSpeeds().duration() / (m_parent->fixtures().size() + 1) * serialNumber();
    }
    else
    {
        return 0;
...
void EFXFixture::nextStep(MasterTimer* timer, QList<Universe *> universes)
{
    ...
    // Nothing to do
    if (m_parent->innerSpeeds().duration() == 0)
        return;

    // Scale from elapsed time in relation to overall duration to a point in a circle
    uint pos = (m_elapsed + timeOffset()) % m_parent->innerSpeeds().duration();
    ...
    if ((m_parent->propagationMode() == EFX::Serial &&
        m_elapsed < (m_parent->innerSpeeds().duration() + timeOffset()))
        || m_elapsed < m_parent->innerSpeeds().duration())
    {
