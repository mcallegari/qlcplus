    ...
    , m_currentHandler(NULL)
{
    ...
    m_bgRect = m_scene->addRect(0, 0, m_scene->width(), m_scene->height(), QPen(Qt::NoPen),
                                QBrush(QColor(70, 70, 70, 255), Qt::SolidPattern));
...
void ChannelModifierGraphicsView::addNewHandler()
{
    ...
    uchar halfDMXval =
        prevHandler->m_dmxMap.second + ((nextHandler->m_dmxMap.second - prevHandler->m_dmxMap.second) / 2);
...
void ChannelModifierGraphicsView::slotItemMoved(HandlerGraphicsItem* item, QGraphicsSceneMouseEvent* event,
                                                QRectF limits)
{
