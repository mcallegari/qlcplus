    ...
    , m_lastInputValue(-1)
{
    ...
    connect(this, SIGNAL(requestSliderUpdate(int)), m_slider, SLOT(setValue(int)));
    ...
    connect(m_cngWidget, SIGNAL(colorChanged(QRgb)), this, SLOT(slotClickAndGoColorChanged(QRgb)));
    connect(m_cngWidget, SIGNAL(levelAndPresetChanged(uchar, QImage)), this,
            SLOT(slotClickAndGoLevelAndPresetChanged(uchar, QImage)));
    connect(this, SIGNAL(monitorDMXValueChanged(int)), this, SLOT(slotMonitorDMXValueChanged(int)));
...
void VCSlider::setChannelsMonitorEnabled(bool enable)
{
    ...
    {
        disconnect(m_resetButton, SIGNAL(clicked(bool)), this, SLOT(slotResetButtonClicked()));
...
void VCSlider::slotUniverseWritten(quint32 idx, const QByteArray &universeData)
{
    ...
    if (mixedDMXlevels == false && monitorSliderValue != m_monitorValue)
    {
...
void VCSlider::setClickAndGoWidgetFromLevel(uchar level)
{
    ...
    if (m_cngType == ClickAndGoWidget::RGB || m_cngType == ClickAndGoWidget::CMY)
    {
        ...
        float   f = 0;
        if (m_slider)
            f = SCALE(float(level), float(m_slider->minimum()), float(m_slider->maximum()),
                      float(0), float(200));
...
void VCSlider::setPlaybackFunction(quint32 fid)
{
    ...
    if (old != NULL)
    {
        ...
        if (old->type() == Function::SceneType)
        {
            disconnect(old, SIGNAL(flashing(quint32, bool)), this,
                       SLOT(slotPlaybackFunctionFlashing(quint32, bool)));
        }
    }
    ...
    if (function != NULL)
    {
        ...
        connect(function, SIGNAL(running(quint32)), this, SLOT(slotPlaybackFunctionRunning(quint32)));
        connect(function, SIGNAL(stopped(quint32)), this, SLOT(slotPlaybackFunctionStopped(quint32)));
...
void VCSlider::notifyFunctionStarting(quint32 fid, qreal functionIntensity)
{
    ...
    if (m_slider != NULL)
    {
        int value = SCALE(1.0 - functionIntensity, 0, 1.0, m_slider->minimum(), m_slider->maximum());
...
void VCSlider::emitSubmasterValue()
{
    ...
    emit submasterValueChanged(
        SCALE(float(m_levelValue), float(0), float(UCHAR_MAX), float(0), float(1)) * intensity());
}
...
void VCSlider::writeDMXLevel(MasterTimer *timer, QList<Universe *> universes)
{
    ...
    if (m_cngType == ClickAndGoWidget::RGB)
    {
        float f = 0;
        if (m_slider)
            f = SCALE(float(m_levelValue), float(m_slider->minimum()), float(m_slider->maximum()),
                      float(0), float(200));
    ...
    if (m_levelValueChanged)
    {
        ...
        while (it.hasNext() == true)
        {
            ...
            if (fader.isNull())
            {
                fader = universes[universe]->requestFader(m_monitorEnabled ? Universe::Override
                                                                           : Universe::Auto);
                ...
                if (m_monitorEnabled)
                {
                    ...
                    connect(fader.data(), SIGNAL(preWriteData(quint32, QByteArray)), this,
                            SLOT(slotUniverseWritten(quint32, QByteArray)));
                }
            }

            FadeChannel *fc =
                fader->getChannelFader(m_doc, universes[universe], lch.fixture, lch.channel);
...
void VCSlider::writeDMXPlayback(MasterTimer *timer, QList<Universe *> ua)
{
    ...
    else
    {
        ...
        {
#if 0
           // temporarily revert #699 until a better solution is found
...
void VCSlider::setTopLabelText(int value)
{
    ...
    else
    {
        ...
        float f = 0;
        if (m_slider)
            f = SCALE(float(value), float(m_slider->minimum()), float(m_slider->maximum()),
                      float(0), float(100));
...
void VCSlider::setSliderValue(uchar value, bool scale, bool external)
{
    ...
    if (scale)
    {
        val = SCALE(float(value), float(0), float(UCHAR_MAX), float(m_slider->minimum()),
...
void VCSlider::updateFeedback()
{
    ...
    if (m_slider)
    {
        if (invertedAppearance() == true)
            ...
            fbv = m_slider->value();
        fbv = int(SCALE(float(fbv), float(m_slider->minimum()), float(m_slider->maximum()),
    ...
    if (tType == QXmlStreamReader::StartElement)
    {
        ...
        do
        {
            if (level_root.name() == KXMLQLCVCSliderChannel)
            {
                ...
                addLevelChannel(static_cast<quint32>(str.toInt()),
                                static_cast<quint32>(level_root.readElementText().toInt()));
            }
            else
            {
                qWarning() << Q_FUNC_INFO
                           << "Unknown slider level tag:" << level_root.name().toString();
...
void VCSlider::LevelChannel::saveXML(QXmlStreamWriter *doc) const
{
    ...
    doc->writeAttribute(KXMLQLCVCSliderChannelFixture, QString::number(this->fixture));
