    ...
    , m_inputCapture(NULL)
{
    ...
    btnSS += "QToolButton:checked { background-color: #D7DE75; border: 1px solid gray; "
             "border-radius: 3px; padding: 3px; } ";
    ...
    m_label->setStyleSheet("QLabel { background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, "
                           "stop: 0 #345D27, stop: 1 #0E1A0A); "
                           "color: "
    ...
    connect(m_volumeSlider, SIGNAL(valueChanged(int)), this, SLOT(slotVolumeChanged(int)));
...
void VCAudioTriggers::enableCapture(bool enable)
{
    ...
    if (enable == true)
    {
        connect(m_inputCapture, SIGNAL(dataProcessed(double *, int, double, quint32)), this,
                SLOT(slotDisplaySpectrum(double *, int, double, quint32)));
