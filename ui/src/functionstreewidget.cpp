...
QTreeWidgetItem *FunctionsTreeWidget::addAlternateSpeeds(quint32 fid, int idx)
{
    ...
    updateAlternateSpeedsItem(item, function, idx);
    blockSignals(false);
    return item;
}
...
void FunctionsTreeWidget::updateAlternateSpeedsItem(QTreeWidgetItem* item, const Function* function, int idx)
{
    Q_ASSERT(item != NULL);
    Q_ASSERT(function != NULL);
    item->setText(COL_NAME, function->alternateSpeedsString(idx));
